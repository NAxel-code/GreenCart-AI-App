...
const MyOrders = () => {
    const [myOrders, setMyOrders] = useState([]); // State to hold user's orders
        ...
        {myOrders.map((order, index) => (
            <div key={index} className='mb-8 p-4 border border-gray-300 rounded'>
                ...
                md:font-medium max-md:flex-col'>
                    ...
                    <span>Total Amount : {currency}{order.mount}</span>
                    </p>
                    {order.items.map((item, index) => (
                        <div key={index}
                        className={`relative bg-white text-gray-500/70 ${order.items.length !== index + 1 && "border-b"}  border-gray-300 flex flex-col md:flex-row md:items-center justify-betweeen p-4 py-5 md:gap-16 w-full max-w-4xl `}>
